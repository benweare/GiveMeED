...
See the paper: https://doi.org/10.1111/jmi.70151
Preprint: http://arxiv.org/abs/2507.10247
Please consider citing if you found this script useful.


###THIS VERSION###
This version has been modified to use the hardware control of stage tilt on a 
JEOL TEM during 3DED, to allow access to different tilt rates.
					...
					number &camera_length )
{
	...
	OKDialog("Press OK and use tilt pad to start 3DED")
	EMSetBeamBlanked(0)//unblank beam
	result("Waiting for tilt to start...\n")
	...
	while ( angle_delta < fiddle )//waits until tilt starts
	{
		...
		angle_delta = abs( current_alpha - start_angle) - 1.0
	}
	...
	if ( alpha_start > 0 )
		while ( abs( EMGetStageAlpha( )- alpha_start ) <= abs( ( alpha_start - alpha_end  ) - fiddle ) )
		{
			...
			//result("Current angle: "+EMGetStageAlpha( )+"\n")
			sleep(0.02)
		}
	else
		while ( abs( EMGetStageAlpha( ) - alpha_start ) <= abs( ( alpha_start - alpha_end  ) + fiddle ) )
